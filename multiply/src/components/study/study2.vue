...
<template>
    <div>
        ...
        <el-row :gutter="60">
          ...
          <el-col :span="6">
            <el-button type="primary" @click="dialogFormVisiblePut = true">发表看法</el-button>
              <!-- 发表内容 -->
...
export default {
  ...
  methods: {
    async getinfo () {
      const { data: res } = await this.$http.post('study/study2/selectcourse', this.queryinfo)
    ...
    async getanswer (id) {
      ...
      const { data: res } = await this.$http.post('study/study2/getanswer', this.queryinfo)
      if (res.status !== 200) return this.$message.console.error(res.message)
    ...
    async giveanswer () {
      ...
      const { data: res } = await this.$http.post('study/study2/giveanswer', this.answerinfo)
      if (res.status !== 200) return this.$message.console.error(res.message)
    ...
    async giveinfo () {
      ...
      const { data: res } = await this.$http.post('study/study2/giveinfo', this.putinfo)
      if (res.status !== 200) return this.$message.console.error(res.message)
    ...
    // 搜索
    async findinfo () {
      const { data: res } = await this.$http.post('study/study2/findinfo', this.queryinfo)
      if (res.status !== 200) return this.$message.console.error(res.message)
      this.info = res.data
      this.total = res.totalpage
    }
